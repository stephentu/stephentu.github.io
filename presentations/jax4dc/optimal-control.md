...
- <!-- .element: class="fragment" -->
  **dynamics** $f : \mathrm{Time} \times \mathrm{State} \times \mathrm{Input} \rightarrow \mathrm{State}$.
- <!-- .element: class="fragment" -->
  **stagewise cost** $c : \mathrm{Time} \times \mathrm{State} \times \mathrm{Input} \rightarrow \mathbb{R}$.
- <!-- .element: class="fragment" -->
  **terminal cost** $c_f: \mathrm{Time} \times \mathrm{State} \rightarrow \mathbb{R}$.
...
$$
\begin{align*}
&\min_{\\{u_t\\}} \sum_{t=0}^{T-1} c(t, x_t, u_t) + c_f(T, x_T) \\\\
&\~\text{s.t.}\~\~x_{t+1} = f(t, x_t, u_t), \quad t \in [T-1] \\\\
&\~\~\~\~\~\~\~\~\\, x_0\~\text{given}.
\end{align*}
$$
...
<p class="fragment"><strong>Output:</strong> The corresponding \(x_1, \dots, x_T \in \mathbb{R}^n\) in 
$$x_{t+1} = f(t, x_t, u_t).$$
</p>
...
**Input:** <!-- .element: class="fragment" -->
1. stagewise/terminal cost functions $(c, c_f)$, <!-- .element: class="fragment" -->
2. a sequence of inputs $U = \\{u_t\\}$, and <!-- .element: class="fragment" -->
3. a trajectory $X = \\{x_t\\}$, <!-- .element: class="fragment" -->

<p class="fragment"><strong>Output:</strong> The (scalar) cost of this trajectory:
$$
...
Let us bundle the stagewise and terminal costs:
...
We can now compose rollout and trajectory cost to write an objective
...
will use `brax`, a Jax-based physics engine (similar to MuJoCo/PyBullet),
to supply dynamics functions.

A <!-- .element: class="fragment" -->
key abstraction in `brax` is the notion of an *environment*.
...
We fix a single environment, the `'reacher'` env.
...
containing *derived* information:
...
```python [1-100|4-6|8-10|12-14|16-17|18|20-21]
def reacher_stagewise_cost(
  ...
  # distance to goal
  state_cost += params.stage_pos * (obs[8] ** 2)
  state_cost += params.stage_pos * (obs[9] ** 2)

  # angular velocity of arm
  state_cost += params.stage_vel * (obs[6] ** 2)
  state_cost += params.stage_vel * (obs[7] ** 2)
...
stage_cost = partial(
    reacher_stagewise_cost, params=ReacherCostParams())
```
...
We
now have the tools in place to write a
*first-order, gradient based solver* for optimal control.


We 
will revisit *second-order methods* shortly.
-->
...
For convenience, we fix:
<pre>
<code data-line-numbers class="python fragment" data-fragment-index="2">cost = Cost(stage_cost, terminal_cost)
...
gradient descent implements the following local search update:
$$
    U_{t+1} = U_t - \eta \nabla J(U_t),
$$
where $J(\cdot)$ represents the objective function.

There are however many variants: momentum, Adam, AdaGrad, etc.
<!-- .element: class="fragment" -->

We will use `optax` to abstract over the specific update rules.
<!-- .element: class="fragment" -->
...
We <!-- .element: class="fragment" -->
can also "drop in" our cost function to Python optimization libraries that accept objective functions as arguments.
...
def scipy_obj(flat_U: np.ndarray) -> tuple[float, np.ndarray]:
  ...
  U = flat_U.reshape(U.shape)  # unflatten
...
<li><p><strong>Iterative LQR with <code>trajax</code>.</strong></p>
<ul>
<li>AD through cost/dynamics.</li>
</ul>
</li>
...
A more robust and stable method is known as *iterative LQR*, which is based
on sequential linearization of the dynamics and quadratization of the cost.
<!-- .element: class="fragment" -->
...
$$
\begin{align*}
  f(t, x, u) &= A_t x + B_t u &&\text{(linear dynamics)}, \\\\
  c(t, x, u) &= \frac{1}{2} \begin{bmatrix} x_t \\\\ u_t \end{bmatrix}^{\mathsf{T}}  \begin{bmatrix} Q_t & S_t \\\\ S_t^{\mathsf{T}} & R_t \end{bmatrix}  \begin{bmatrix} x_t \\\\ u_t \end{bmatrix} &&\text{(quadratic cost)} \\\\
  &\qquad + q_t^{\mathsf{T}} x_t + r_t^{\mathsf{T}} u_t.
\end{align*}
$$

LQR <!-- .element: class="fragment" --> is solved by backward Ricatti iteration,
...
- Linearize the dynamics and quadratize the cost around the current trajectory. <!-- .element: class="fragment" -->
- Solve resulting LQR problem for gains. <!-- .element: class="fragment" -->
- Update trajectory using computed gains via line-search. <!-- .element: class="fragment" -->
...
Linearized <!-- .element: class="fragment" --> dynamics:
$$
...
the parameters that are the most robust over a set of ICs.
...
def meta_cost(
  ...
  trajax_cost = make_trajax_cost(Cost(
      partial(reacher_stagewise_cost, params=params),
...
We now look at an advanced feature of `trajax`'s ilqr:
differentiating solutions w.r.t.
parameters of the dynamics and cost.
<!-- .element: class="fragment" -->

<p class="fragment">
To
simplify things, we consider acrobot dynamics:<br/>
...
**Q:** Which dynamics parameter is the optimal control
effort $\sum_{t=0}^{T-1} \\|u_t\\|^2$ most sensitive to?
<!-- .element: class="fragment" -->
